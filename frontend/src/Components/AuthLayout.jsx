    ...
    useEffect(() => {
        const unsubscribe= onAuthStateChanged(auth,async userData=>{
            ...
            if(!(userInfo&&userInfo.email)&&userData){
                const response=await axios.post(`http://localhost:3000/api/v1/user/getuserdata`,{email:userData.email},
                    {
                ...
                const data= {name:(userInfo.name?response.data.name:userData.displayName),email:(userInfo.email?response.data.email:userData.email), profileImage:((response.data&&response.data.profileImage)?'http://localhost:3000/'+response.data.profileImage:userData.photoURL), mobileNumber:response.data?response.data.mobileNumber:'', status:response.data?response.data.status:''}
                console.log(data)
