...
function Profiles() {
    ...
    const sendRequest=async()=>{
        ...
        const response=await axios.post(`http://localhost:3000/api/v1/user/sendrequest`,{email:email, senderEmail:senderEmail, socketId:sessionStorage.getItem('socketId')},
            {
    ...
    useEffect(() => { 
       async function getUserData(){
            const response=await axios.post(`http://localhost:3000/api/v1/user/getuserdata`,{email:email,partial:true},            {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    const confirmRequest=async()=>{
        const response=await axios.post(`http://localhost:3000/api/v1/user/confirmrequest`,{email:email},
            {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json'
    ...
    const withdrawRequest=async()=>{
        const response=await axios.post(`http://localhost:3000/api/v1/user/withdrawrequest`,{email:email},
            {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json'
    ...
    const unfollow=async()=>{
        const response=await axios.post(`http://localhost:3000/api/v1/user/unfollow`,{email:email},
            {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json'
    ...
    const message=async()=>{
        const chatCreated=await axios.post(`http://localhost:3000/api/v1/messages/createChatId`,{email:email},
            {
                withCredentials: true,
                headers: {
                    'Content-Type': 'application/json'
        ...
        const createContacts=await axios.post(`http://localhost:3000/api/v1/messages/setcontacts`,{email:email, name:userData.name},{
            withCredentials: true,
            headers: {
                'Content-Type': 'application/json'
            }
...
<div className="flex flex-col items-center p-4 bg-white shadow rounded-lg">
    <img className="rounded-full w-32 border-4 border-gray-300" src={'http://localhost:3000/' + userData.profileImage} alt="Profile" />
